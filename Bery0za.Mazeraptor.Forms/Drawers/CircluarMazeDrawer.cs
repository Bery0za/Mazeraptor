...
namespace Bery0za.Mazerator.Forms.Drawers
{
	...
	{
        protected override void FillMaze(Graphics graphics, CircularStructure structure, DrawingParameters parameters, IEnumerable<Cell> order)
        {
            float translate = parameters.strokeWidth + structure.parameters.rings * parameters.cellSize + parameters.cellSize * 0.5f;
            ...
            foreach (Cell cell in order)
            {
                ...
                float sweep = CircularFunctions.RadToDeg(polarSideB.angle) - CircularFunctions.RadToDeg(polarSideA.angle);
        ...
        protected override (int width, int height) CalculateBitmapSize(CircularStructure structure, DrawingParameters parameters)
        {
            int diameter = (int)((structure.parameters.rings * 2 + 1) * parameters.cellSize + 2 * parameters.strokeWidth);
