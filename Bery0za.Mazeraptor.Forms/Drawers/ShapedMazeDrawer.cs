...
namespace Bery0za.Mazerator.Forms.Drawers
{
    public class ShapedMazeDrawer : MazeDrawer<ShapedStructure>
    {
        ...
        protected override void StrokeMaze(Graphics graphics, ShapedStructure structure, DrawingParameters parameters)
        {
            graphics.TranslateTransform(parameters.strokeWidth, parameters.strokeWidth);

            float width = structure.parameters.shape.Width;
            float height = structure.parameters.shape.Height;
            float drawWidth = parameters.cellSize;
            float drawHeight = parameters.cellSize;
            ...
            for (int x = 0; x < width; x++)
            {
                ...
                {
                    if (!structure.ContainsCellAtPosition(new RectangularPosition(x, y))) continue;
