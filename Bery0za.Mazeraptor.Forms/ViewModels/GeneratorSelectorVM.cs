...
namespace Bery0za.Mazerator.Forms.ViewModels
{
    ...
    {
        private static Dictionary<CellSelector, MazeGenerator.CellSelector> _cellSelectors =
            new Dictionary<CellSelector, MazeGenerator.CellSelector>()
            {
        ...
        private static Dictionary<CellSelector, Func<float, MazeGenerator.CellSelector>> _parametrizedSelectors =
            new Dictionary<CellSelector, Func<float, MazeGenerator.CellSelector>>()
            {
        ...
        private static Dictionary<NeighborSelector, MazeGenerator.NeighborSelector> _neighborSelectors =
            new Dictionary<NeighborSelector, MazeGenerator.NeighborSelector>()
            {
                { NeighborSelector.Random, MazeGenerator.RandomNeighbourSelector },
            };

        public readonly ContextWrapper<IGeneratorVM> Generator =
            new ContextWrapper<IGeneratorVM>(new RecursiveBacktrackerGeneratorVM());

        public readonly PropertyWrapper<GeneratorType> Type =
            new PropertyWrapper<GeneratorType>(GeneratorType.RecursiveBacktracker);
        ...
        internal static MazeGenerator.NeighborSelector CreateNeighborSelector(
            NeighborSelector type,
            float parameter = 1)
        {
            switch (type)
            {
